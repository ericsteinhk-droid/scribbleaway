...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribe(
    ...
    // High-accuracy configuration tuned for Canadian French audio files.
    //
    // Beam search (rather than greedy) meaningfully improves word-error rate at
    // the cost of extra compute — acceptable for offline file transcription.
    // The language is forced to French so short/ambiguous clips are never
    // misdetected as English, and translation is disabled so the transcript
    // stays in French (Québécois vocabulary and all).
    ...
    // Beam width of 5 is the accuracy sweet spot for the base model.
    params.beam_search.beam_size = 5;
    params.greedy.best_of        = 5;

    // Temperature fallback: start deterministic, only raise temperature when the
    // decoder trips the compression-ratio / log-prob guards. This suppresses the
    // repetition loops Whisper can fall into on music, silence or noise.
    ...
    LOGI("About to run whisper_full (fr, beam=5, threads=%d, samples=%d)", num_threads, audio_data_length);
